...
delete_files() {
    ...
    if [[ "$pattern" == *"."* ]]; then
        ...
        if [[ -n "${file_types[$ext]}" ]]; then
            ...
            echo "${file_types[$ext]}"
            find "$DOWNLOADS_DIR" -name "*.$ext" -exec mv {} "$TRASH_DIR" \;
        else
        ...
        find "$DOWNLOADS_DIR" -regex "$pattern" -exec mv {} "$TRASH_DIR" \;
    fi
}
...
while true; do
    ...
    case $action in
        r|R)
            ...
            ;;
        d|D)
            echo "Enter the file extension, full name, or regex pattern to delete:"
            read -r pattern
